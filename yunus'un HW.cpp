...
void f5(char s[dim]) { // hata var "hamam ara beni" --çözüldü.
	int p, i, a, counter = 0, wsize[dim / 2] = { 0 }, wlenght[dim / 2] = { 0 }, t[dim / 2] = { 0 };// Dikkat !
	for (i = 1; s[i - 1] != '\0'; i++) {//i 1 arttý
	...
	for (int k = 1; k < p + 1; k++) {
		for (int j = 1; j < p + 1; j++) {
			if (t[j] == p - k) { // Puaný en yüksek olaný ilk yazýyor.---------------HHep ilk girilen harfi yazýyor.---hata--çözüldü
...
void fprint(char s[dim], int size) {
	int i;
	printf("\nThe input is :\n ");
	for (i = 0; i < size; i++) { //Diziler 0 dan baþlar s[0]. Bu sebeple i=0'dan baþlýyoruz.
		printf("%c", s[i]);
	}
}
